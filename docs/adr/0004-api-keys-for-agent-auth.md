...
The agent-collaboration MVP (shipped earlier) introduced agent authentication by returning the user's session cookie value as `agent_token` from `POST /api/v1/boards`. The agent uses it as `Authorization: Bearer <agent_token>` on subsequent requests. This works as a foothold but has real shortcomings:
