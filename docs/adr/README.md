...
- [ADR-0023: Staging is a named environment; previews stay throwaway](0023-staging-is-a-named-environment.md) — a Vercel custom environment on a `staging` branch holds one stable domain for Stripe's webhook and Auth0's callback, with deployment protection off and nothing bypassed; only an unnamed preview still builds its OAuth callback from `VERCEL_URL`, keyed on `VERCEL_TARGET_ENV`
- [ADR-0024: Every query waits for the schema build](0024-queries-wait-for-the-schema-build.md) — `db_config.ts` starts `initializeDatabase()` once at load and `pool.query`/`pool.connect` await it; the build has its own ungated `schemaPool`; `db_init.ts` no longer runs on import, because Vercel froze the background build mid-transaction and an empty database was never created
- [ADR-0025: Staging is its own Vercel project](0025-staging-is-its-own-vercel-project.md) — supersedes ADR-0023 §1 and §4; `all_except_custom_domains` only exempts production custom domains, so staging is `retrograde-staging` with `staging` as its production branch; previews keep Vercel's login wall; nothing is bypassed
