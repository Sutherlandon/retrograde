...
4. **Entitlement is `active` only.** A `past_due` renewal (still inside Stripe's retry window) closes the gate immediately. One-line change in `entitlements.ts` if a grace period is wanted. The same rule means **discounts must be coupons, never Stripe free trials**: a trial subscription is `trialing`, which grants nothing and, as a non-`active` status, revokes the crew's API keys. A 100%-off promotion code keeps the subscription `active`; Checkout accepts codes, and a session with nothing to charge (`no_payment_required`) activates like a paid one.
