...
**On a self-hosted instance every account is tier 3** (`SELF_HOSTED=true`, ADR-0016). The entitlement seam returns true without reading billing state, so CREW-002 always allows and the ADR-0015 lapse freeze never applies. Billing and scheduled cleanup do not exist there: CREW-020, CREW-021, API-006 and API-007 return 404, and crewless boards are never archived (ADR-0020). The dashboard is home and the marketing site does not render (SITE-001, SITE-002, SITE-005, SITE-007; ADR-0017), and robots.txt disallows everything (SITE-008). There is no tier 1 there either: every page and API call needs a signed-in account, or an API key for the JSON API, so no guest user or anonymous board is ever created (ADR-0021).
